...
function getReliabilityColor(score: number): string {
  ...
  return 'text-brand-red dark:text-red-500';
}
const prepareChartData = (shapData: MatchExplanation[] = []) => {
  /* ... */ return shapData
...
const RenderImpactIcon = (props: any) => {
  /* ... */ const { x, y, width, value } = props;
  const iconSize = 10;
  const iconX = x + width + 5;
  const iconY = y - iconSize / 2 + 6;
  if (value > 0) {
    return <TrendingUp x={iconX} y={iconY} size={iconSize} className="text-green-600" />;
  } else if (value < 0) {
    return <TrendingDown x={iconX} y={iconY} size={iconSize} className="text-red-500" />;
  }
  return null;
};
const CustomTooltip = ({ active, payload, label }: any) => {
  /* ... */ if (active && payload && payload.length) {
    ...
    let valueDisplay = data.value.toFixed(2);
    if (label === 'Reliability') valueDisplay = `${(data.value * 100).toFixed(0)}%`;
    if (label === 'Location') valueDisplay = data.value === 1 ? 'Local' : 'Not Local';
    return (
      <div className="bg-card p-2 border border-border rounded shadow-lg text-xs">
        {' '}
        <p className="font-bold text-foreground">{label}</p>{' '}
        <p className={`font-semibold ${data.impact >= 0 ? 'text-green-600' : 'text-red-500'}`}>
          {' '}
          {`Impact: ${data.impact > 0 ? '+' : ''}${data.impact.toFixed(3)}`}{' '}
        </p>{' '}
        <p className="text-muted-foreground">{`Value: ${valueDisplay}`}</p>{' '}
      </div>
    );
  }
  return null;
  ...
  const handleContact = async () => {
    ...
    try {
      ...
      toast.success('Notification Sent!', {
        description: `${match.donor.full_name} notified (simulated).`,
      });
    } catch (err) {
      toast.error('Error', {
      ...
      <CardFooter className="p-3 bg-muted/30 border-t">
        <Dialog>
          <DialogTrigger asChild>
            <Button
              className="w-full bg-brand-sky hover:bg-brand-sky/90 text-white font-semibold rounded-md shadow hover:shadow-md transition"
              size="sm"
            >
          ...
          <DialogContent>
            ...
            <DialogHeader>
              ...
              <DialogDescription>
                {' '}
                Send secure notification to {match.donor.full_name}?{' '}
              </DialogDescription>{' '}
            </DialogHeader>{' '}
            <DialogFooter>
              {' '}
